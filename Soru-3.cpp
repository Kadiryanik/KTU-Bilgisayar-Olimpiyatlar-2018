...
// C++ used for struct constructers
...
int main(int argc, char *argv[]){
  ...
  while (offset < size){
    if(sscanf(buffer + offset, "%d %d %d %d", &coordinates[i++], &coordinates[i++], &coordinates[i++], &coordinates[i++]) == 4){
      counter++;
    }
    while (buffer[offset++] != '\n' && offset < size);
  }
  ...
  file = fopen("Soru-3-output.txt", "w+");
